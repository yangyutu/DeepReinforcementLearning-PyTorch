...
from Env.CustomEnv.TimedMaze.TimeMaze import TimeMazeEnv
...
env = TimeMazeEnv(config)

N_S = env.stateDim[0]
N_A = env.nbActions

timeIndexMap = {}
for i in range(config['episodeLength'] + 5):

    count = -1
    for idx, kink in enumerate(config['rewardKinks']):
        if i < kink:
            timeIndexMap[i] = idx
            count = idx
            break

    if i not in timeIndexMap:
        timeIndexMap[i] = count + 1

numNets = len(set(timeIndexMap.values()))

policyNets = [MulChanConvNet(N_S, 128, N_A) for _ in range(numNets)]
targetNets = [deepcopy(net) for net in policyNets]
optimizers = [optim.Adam(policyNets[i].parameters(), lr=config['learningRate']) for i in range(numNets)]


agent = StackedDQNAgent(config, policyNets, targetNets, env, optimizers, torch.nn.MSELoss(reduction='none'), N_A,
                        stateProcessor=stateProcessor, timeIndexMap=timeIndexMap)
...
if trainFlag:
    ...
    if plotPolicyFlag:
        ...
        for phiIdx in range(8):
            ...
            np.savetxt('DynamicMazePolicyBeforeTrain' + config['mapName'] +'phiIdx' + str(phiIdx) + '.txt', policy,
                       fmt='%d', delimiter='\t')
            np.savetxt('DynamicMazeValueBeforeTrain' + config['mapName'] + 'phiIdx' + str(phiIdx) + '.txt', value,
                       fmt='%.3f', delimiter='\t')
            ...
            for i in range(policy.shape[0]):
                for j in range(policy.shape[1]):
                    if env.mapMat[i, j] == 1:
                        ...
                        Qvalue = agent.policyNet(stateProcessor([state], config['device'])[0])
                        value[i, j] = Qvalue[0, policy[i, j]].cpu().item()
            np.savetxt('DynamicMazePolicyAfterTrain' + config['mapName'] + 'phiIdx' + str(phiIdx) + '.txt', policy,
                       fmt='%d',
                       delimiter='\t')
