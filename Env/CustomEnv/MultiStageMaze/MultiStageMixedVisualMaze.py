...
class MultiStageMixedVisualMaze:
    ...
    def getHindSightExperience(self, state, action, nextState, info):
        ...
        if self.hindSightInfo['obsFlag'] or action == 0 or stageID == 0:
            ...
            state = {'state': {'sensor': np.expand_dims(sensorInfoMat, axis=0),
                     'target': np.array(
    ...
    def rewardCal(self):
        ...
        if self.stepCount <= self.endStep and self.stageID == (self.numStages - 1) \
                and self.is_terminal(distance):
    ...
    def step(self, action):
        ...
        if self.stageID == 1:
            if action == 1:
                ...
                jmRaw = np.array([random.gauss(0, self.xyStd) + 2.0,
        ...
        if self.currentState[1] > self.stageRatio * self.mapMat.shape[1] and self.stageID == 0:
            # pass job to agent 1
        ...
        if self.stageID == 1 and (self.currentState[1] + dy) <  0.9 * self.stageRatio * self.mapMat.shape[1]:
            jm = np.array([0.0, 0.0, jmRaw[2]], dtype=np.float32)
        elif self.obsMap[i, j] == 0:
    ...
    def reset_helper(self):
        ...
        if self.currentState[1] > self.stageRatio * self.mapMat.shape[1]:
            self.stageID = 1
        else:
            self.stageID = 0
