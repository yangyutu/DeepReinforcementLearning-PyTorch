...
class MultiStageMixedMaze:
    ...
    def step(self, action):
        ...
        if self.currentState[1] > 0.5 * self.mapMat.shape[1] and self.stageID == 0:
            # pass job to agent 1
    ...
    def reset_helper(self):
        ...
        if self.config['dynamicInitialStateFlag']:
            while True:
                if self.stageID == 0:
                    col = random.randint(0, int(0.5 * self.mapMat.shape[1]) - 1) + self.padding
                    row = random.randint(0, self.mapMat.shape[0] - 1) + self.padding
                if self.stageID == 1:
                    col = random.randint(int(0.5 * self.mapMat.shape[1]) + 1, self.mapMat.shape[1] - 1) + self.padding
                    row = random.randint(0, self.mapMat.shape[0] - 1) + self.padding
                ...
                if np.sum(self.obsMap[row - 2:row + 3,
                          col - 2:col + 3]) == 0 and not self.is_terminal(distanctVec):
                    break
            # set initial state
