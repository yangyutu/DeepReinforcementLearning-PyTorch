...
from Env.CustomEnv.MultiStageMaze.MultiStageFreeMaze import CooperativeSimpleMazeTwoDContinuous
...
from Agents.DDPG.DDPGMultiStageUnit import DDPGMultiStageUnit
...
env = CooperativeSimpleMazeTwoDContinuous(config)
N_S = env.stateDim
N_A = env.nbActions
...
for i in range(config['numStages']):
    ...
    agent = DDPGMultiStageUnit(config, actorNets, criticNets, env, optimizers, torch.nn.MSELoss(reduction='mean'), N_A)
    agents.append(agent)
