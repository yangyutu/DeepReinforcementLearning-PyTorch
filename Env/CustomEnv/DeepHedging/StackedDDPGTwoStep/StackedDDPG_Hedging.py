...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        x = F.relu(self.linear2(x))
        action = torch.tanh(self.linear3(x))*2
...
for n in range(2):
    ...
    for s in S:
        ...
        combinedState = {'state': state, 'timeStep': n}

        action = agent.select_action(agent.actorNets[n], combinedState, False)
        result.append([s, action, action / s])

    np.savetxt("resultBeforeTrain" + str(n) + ".txt", np.array(result))
...
for n in range(2):
    ...
    np.savetxt("resultAfterTrain" + str(n) + ".txt", np.array(result))