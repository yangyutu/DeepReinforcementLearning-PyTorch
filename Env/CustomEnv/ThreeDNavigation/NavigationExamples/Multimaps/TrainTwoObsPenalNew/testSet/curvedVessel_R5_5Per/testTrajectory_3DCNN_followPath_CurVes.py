...
def obstacleConstructorCallBack(configName = 'config_RBC.json'):
    ...
    for i in range(config['numObstacles']):
        name = 'obs' + str(i)
        obstacles.append(RBCObstacle(np.array(config[name]['center']), config[name]['scale']*0.8, np.array(config[name]['orient'])))
...
config['gravity'] = 0
config['multiMapNames'] = ['config_RBC_CV_R5_5PerTest.json']
