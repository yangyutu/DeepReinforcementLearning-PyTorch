...
def obstacleConstructorCallBack(configName = 'config_RBC.json'):
    ...
    for i in range(config['numObstacles']):
        name = 'obs' + str(i)
        obstacles.append(RBCObstacle(np.array(config[name]['center']), config[name]['scale']*1.0, np.array(config[name]['orient'])))
...
checkpoint = torch.load('../../Log/Epoch50000_checkpoint.pt')
...
config['gravity'] = 0
config['multiMapNames'] = ['config_RBC_CV_R15_10PerTest.json']
